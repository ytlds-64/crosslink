...
> 当前进度：**M1 — 跨平台加密传输骨架**（TCP 连通 + X25519/AES-GCM 加密握手 + 心跳）。
> 输入捕获 / 注入（鼠标键盘真正转发到对端）在 **M2** 实现，本版仅验证安全通道与连通性。
...
- **M1**（当前）：传输骨架 — TCP 连通 + 加密握手 + 心跳 ✅
- **M2**：输入注入 — 单机内捕获并回放（Windows `SendInput` / Raw Input；macOS `CGEventTap` / `CGEventPost`）
- **M3**：单向控制 — Server 捕获 → Client 注入
- **M4**：边缘切换 — 屏幕几何 + 光标越界接管 + 反向回切
- **M5**：键码翻译 — HID 中间码，覆盖常用键 + 修饰键
- **M6**：发现 / 配置 / GUI — mDNS、指纹授权、设置界面
- **M7**：增强 — 剪贴板共享、文件拖拽、加密加固、打包发布

---

## 已知限制

- M1 阶段不转发任何真实键鼠输入，仅验证安全通道与连通性。
- 当前为单一 TCP 连接；M4 起会引入多屏幕几何与边缘逻辑。
